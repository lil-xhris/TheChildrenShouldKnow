...
export default function EditWriter() {
  const { user, isLoading, isAdmin, getWriterById, updateWriter } = useAuth()
    ...
    if (user && isAdmin() && writerId) {
      ...
      if (writer) {
        setFormData({
          ...
          bio: writer.bio,
          image: writer.image,
          works: writer.works?.length ? writer.works : [""],
          achievements: writer.achievements?.length ? writer.achievements : [""],
        })
        setImagePreview(writer.image)
        setIsLoaded(true)
      } else {
        router.push("/admin-dashboard/writers")
      }
    }
  }, [user, isLoading, router, isAdmin, writerId, getWriterById])
  ...
  const handleSubmit = (e) => {
    ...
    try {
      updateWriter(writerId, formData)
      router.push("/admin-dashboard/writers")
    } catch (error) {
      console.error("Error updating writer:", error)
      setErrors({ submit: "Failed to update writer. Please try again." })
    } finally {
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="p-6 border-b flex items-center">
              ...
              <div>
                <h1 className="text-2xl font-bold text-purple-700">Edit Writer</h1>
                <p className="text-gray-600">Update writer information</p>
              </div>
            </div>
